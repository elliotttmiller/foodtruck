...
import { authorizeKiosk, clearLiveSession, ensureSession, fetchLiveOrders, loadLiveSession, requestSquareSync, setOrderStatus, subscribeToOrders } from '../lib/liveOrders.js';
...
export function LiveOrdersPage(){
  ...
  useEffect(()=>{let cancelled=false;(async()=>{if(!session)return;try{const fresh=await ensureSession(session);if(cancelled)return;if(fresh.accessToken!==session.accessToken)setSession(fresh);await load(fresh.accessToken);}catch(err){if(!cancelled)setError(err.message);}})();return()=>{cancelled=true;};},[session?.accessToken,load]);
  useEffect(()=>{if(!authorized||!session?.accessToken)return;const stop=subscribeToOrders(session.accessToken,()=>load(session.accessToken),setConnection);const poll=setInterval(()=>load(session.accessToken),15000);const timer=setInterval(()=>tick(v=>v+1),1000);return()=>{stop();clearInterval(poll);clearInterval(timer);};},[authorized,session?.accessToken,load]);
  ...
  const move=async(order,status)=>{setBusyId(order.id);setError('');const previous=orders;setOrders(list=>status==='complete'?list.filter(x=>x.id!==order.id):list.map(x=>x.id===order.id?{...x,status,ready_at:status==='ready'?new Date().toISOString():x.ready_at}:x));try{await setOrderStatus(session.accessToken,order.id,status);setLastUpdated(new Date());}catch(err){setOrders(previous);setError(err.message);}finally{setBusyId('');}};
