...
import { AlertTriangle, Check, ChefHat, Clock3, Expand, LockKeyhole, Plus, RefreshCw, Wifi, WifiOff, X } from 'lucide-react';
...
function OrderDetail({order,busy,onClose,onMove}){
  ...
  return <div className="order-dialog-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)onClose();}}>
    <section className={`order-dialog ${isActive?'':'is-ready'}`} role="dialog" aria-modal="true" aria-labelledby="order-dialog-title" ref={dialogRef}>
      <div className="order-dialog-header"><div className="order-dialog-top"><span className="order-dialog-state">{isActive?<Clock3 size={16}/>:<Check size={16}/>} {isActive?'ACTIVE ORDER':'READY FOR PICKUP'}</span><button className="order-dialog-x" type="button" aria-label="Close order details" onClick={onClose} ref={closeRef}><X size={22}/></button></div><div className="order-dialog-heading"><h2 id="order-dialog-title">#{order.ticket_number} <span>{order.customer_name||'Walk-up'}</span></h2><time dateTime={received}><Clock3 size={23}/>{elapsed(received)}</time></div><p>Received {received?serviceTime.format(new Date(received)):'—'} <span aria-hidden="true">·</span> {itemCount(order.items)} {itemCount(order.items)===1?'item':'items'}</p></div>
      ...
      <div className="order-dialog-footer"><button className="order-dialog-close" type="button" onClick={onClose}>Close</button><button className={isActive?'ready-button':'complete-button'} type="button" disabled={busy} onClick={()=>onMove(order,isActive?'ready':'complete')}>{busy?<RefreshCw size={19} className="spin"/>:isActive?<ChefHat size={19}/>:<Check size={19}/>} {isActive?'Mark Ready':'Complete Order'}</button></div>
    </section>
  </div>;
}
...
export function LiveOrdersPage({demo=false}){
  ...
  return <main className="live-board">
    <header className="live-header">
      ...
      <div className={`live-connection ${demo?'demo':connection}`} role="status" aria-live="polite"><span className="status-dot"/>{demo?<Check size={17}/>:connection==='connected'?<Wifi size={17}/>:<WifiOff size={17}/>}<div><strong>{demo?'Preview only':connection==='connected'?'Realtime connected':connection==='offline'?'Offline — use backup tickets':'Reconnecting'}</strong><span>{demo?'Sample tickets · no Square connection':lastUpdated?`Database updated ${elapsed(lastUpdated)} ago`:'Waiting for database update'}</span></div></div>
      <div className="live-tools">{!demo?<a className="live-quiet" href="#/live-orders-demo">Preview orders</a>:null}<button type="button" className="live-quiet" onClick={sync} disabled={syncing}>{demo?<Plus size={17}/>:<RefreshCw size={17} className={syncing?'spin':''}/>}<span>{demo?'Add test order':syncing?'Syncing…':'Refresh from Square'}</span></button><button type="button" className="live-quiet live-exit" onClick={logout}>{demo?null:<LockKeyhole size={16}/>}<span>{demo?'Exit preview':'Lock'}</span></button></div>
    </header>
    {demo?<div className="live-banner preview" role="status"><AlertTriangle size={17}/><span><strong>PREVIEW ONLY</strong> — Sample tickets stay in this browser tab. They are not Square sales and are never saved.</span><span className="banner-tag">PREVIEW MODE</span></div>:null}
    {error?<div className="live-banner error" role="alert"><AlertTriangle size={17}/><span>{error}</span></div>:null}
    <div className="live-lanes">
      <section className="live-lane active-lane" aria-labelledby="active-title"><div className="live-section-heading"><div><h2 id="active-title">Active Orders <span>{active.length}</span></h2><p>Oldest first. Mark Ready when the full order is complete.</p></div></div>
        <div className="live-lane-scroll"><div className="active-grid">{active.length?active.map(order=><article className={`order-card ${urgency(order)}`} key={order.id}><button className="order-card-open" type="button" onClick={()=>setSelectedId(order.id)} aria-label={`View full order #${order.ticket_number} for ${order.customer_name||'Walk-up'}`}><div className="order-top"><div><strong className="ticket">#{order.ticket_number}</strong><h3>{order.customer_name||'Walk-up'}</h3><span>{itemCount(order.items)} {itemCount(order.items)===1?'item':'items'}</span></div><div className="order-top-actions"><time dateTime={order.source_created_at||order.created_at}><Clock3 size={16}/>{elapsed(order.source_created_at||order.created_at)}</time><Expand size={16} aria-hidden="true"/></div></div><OrderItems items={order.items}/></button><button type="button" className="ready-button" disabled={busyId===order.id} onClick={()=>move(order,'ready')}>{busyId===order.id?<RefreshCw className="spin" size={18}/>:<ChefHat size={18}/>}Mark Ready</button></article>):<div className="live-empty">No active orders. New paid tickets will appear here.</div>}</div></div>
      </section>
      <section className="live-lane ready-lane" aria-labelledby="ready-title"><div className="live-section-heading"><div><h2 id="ready-title">Ready for Pickup <span>{ready.length}</span></h2><p>Hand out the order, then mark Complete.</p></div></div>
        <div className="live-lane-scroll"><div className="ready-grid">{ready.length?ready.map(order=><article className="pickup-card" key={order.id}><button className="pickup-card-open" type="button" onClick={()=>setSelectedId(order.id)} aria-label={`View full order #${order.ticket_number} for ${order.customer_name||'Walk-up'}`}><div className="pickup-ticket"><strong>#{order.ticket_number}</strong><time dateTime={order.ready_at||undefined}><Clock3 size={14}/> Ready {elapsed(order.ready_at)}</time></div><h3>{order.customer_name||'Walk-up'}</h3><div className="pickup-summary">{order.items?.length?order.items.map((item,index)=><span key={item.uid||`${item.name}-${index}`}>{item.quantity||1} × {item.name||'Item'}</span>):<span>{itemCount(order.items)} items</span>}</div></button><button type="button" className="complete-button" disabled={busyId===order.id} onClick={()=>move(order,'complete')}><Check size={18}/>Complete</button></article>):<div className="live-empty compact">Nothing waiting for pickup.</div>}</div></div>
      </section>
    </div>
