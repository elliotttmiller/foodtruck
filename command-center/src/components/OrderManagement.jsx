...
export function OrderManagement({orders,token,onClose,onChanged}){
  ...
  return <div className="order-dialog-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget&&!busy)onClose();}}><section className="manage-dialog" role="dialog" aria-modal="true" aria-labelledby="manage-title" ref={dialog}>
    <header className="manage-dialog-head"><div><span className="manage-eyebrow">BOARD TOOLS</span><h2 id="manage-title">Manage orders</h2><p>Clear stale tickets without deleting records or changing Square.</p></div><button type="button" className="order-dialog-x" onClick={onClose} disabled={busy} aria-label="Close order management" ref={close}><X size={20}/></button></header>
    <div className="manage-body">
      <section className="manage-panel" aria-labelledby="manage-clear-title"><div className="manage-panel-head"><div><span>WORKING BOARD</span><h3 id="manage-clear-title">Clear tickets</h3></div><strong>{eligible.length} selected</strong></div>
        ...
        <div className="manage-review"><strong>{eligible.length?`${eligible.length} ${eligible.length===1?'ticket':'tickets'} match`:'No matching tickets'}</strong><span>{eligible.length?eligible.slice(0,12).map(o=>`#${o.ticket_number}`).join(' · ')+(eligible.length>12?' · …':''):'Adjust the filters above to select tickets.'}</span></div>
        {confirm?<div className="manage-confirm" role="group" aria-label="Confirm clearing tickets"><strong>Clear {reviewed.length} {reviewed.length===1?'ticket':'tickets'}?</strong><p>They disappear from every live board, remain in history, and can be restored below.</p><div><button type="button" className="live-quiet" onClick={()=>setConfirm(false)} disabled={busy}>Cancel</button><button type="button" className="manage-danger" disabled={busy} onClick={clear}>{busy?'Clearing…':`Clear ${reviewed.length}`}</button></div></div>:<button type="button" className="manage-danger" disabled={!eligible.length||busy} onClick={()=>{setReviewed(eligible);setConfirm(true);}}>Review clear</button>}
        {message?<p className="manage-message" role="status">{message}</p>:null}
      </section>
      <section className="manage-history" aria-labelledby="manage-history-title"><div className="manage-history-head"><div><span>RECOVERY</span><h3 id="manage-history-title">Recently cleared</h3></div><strong>{recent.length}</strong></div><p>Restore a ticket to its previous lane if it was cleared by mistake.</p><div className="manage-history-list">{recent.length?recent.map(o=><div className="manage-history-row" key={o.id}><div className="manage-history-copy"><strong>#{o.ticket_number}</strong><small>{o.cleared_at?new Date(o.cleared_at).toLocaleString():''} · {o.cleared_from_status}</small></div><button type="button" disabled={busy||!o.cleared_from_status} className="manage-restore" onClick={()=>restore(o)}><RotateCcw size={14} aria-hidden="true"/><span>Restore</span></button></div>):<div className="manage-history-empty">No recently cleared tickets.</div>}</div></section>
    </div></section></div>;
}
